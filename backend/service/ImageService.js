...
// revision when the user destroys an image so the change propagates to
...
async function deleteImageById(imageId) {
    ...
    // revision after the image row is gone. The instance still has it
    // here; `image.destroy()` later inside the tx clears it.
    const articleId = image.articleId;
